<?
	...
	include_once "Entertainment_Custom.ips.php";
		...
		if ($CommProperties !== false) {
			...
			switch ($CommType) {
				...
				case c_Property_CommRemSrc:
					...
					Entertainment_SetRemoteControlType($ControlId, $Value, c_Control_RemoteSource);
					break;
	...
	function Entertainment_SendDataBySourceIdx($RoomId, $SourceIdx, $MessageType) {
		...
		$SourceConf  = get_SourceConfiguration();
		foreach ($DeviceTypes as $DeviceType => $DeviceNames) {
			$SourcesData  = $SourceConf[$RoomName][$SourceIdx][$DeviceType];
			
			// wrap older configuration in an array for downward compatibility
